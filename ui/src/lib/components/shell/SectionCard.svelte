<script lang="ts">
	...
	const titleId = $derived(title.toLowerCase().replace(/[^a-z0-9]+/g, '-'));
</script>
...
<section class="bg-[var(--shell-surface)] border border-[var(--shell-line)] rounded-[0.35rem] flex flex-col overflow-hidden">
	<header class="flex justify-between items-center px-3 py-[0.65rem] border-b border-[var(--shell-line)] bg-[var(--shell-surface-muted)]">
		<div class="flex items-center gap-2">
			...
			<h3 id="{titleId}-heading" class="text-[length:var(--text-xs)] font-bold uppercase tracking-[0.05em] text-[var(--shell-text-muted)] m-0">{title}</h3>
			{#if badgeLabel}
	...
	{#if !collapsed}
		<div id="{titleId}-content" role="region" aria-labelledby="{titleId}-heading" class="p-3 text-[length:var(--text-sm)]">
			{@render children()}
		</div>
	{/if}
</section>
