			...
			{#snippet cell({ column, row })}
				...
				{:else if column.key === 'actions'}
					<div class="action-strip">
						<button type="button" class="btn-icon" onclick={() => onedit?.(row as unknown as UserItem)} title="MODIFY_ENTITY" aria-label="Edit user {row.username}">
							<Pencil size={12} />
						</button>
						<button type="button" class="btn-icon danger" onclick={() => ondelete?.(row as unknown as UserItem)} title="PURGE_ENTITY" aria-label="Delete user {row.username}">
							<Trash2 size={12} />
						</button>
					</div>
				{:else}
