			...
			{#snippet cell({ column, row })}
				...
				{:else if column.key === 'actions'}
					<div class="action-strip">
						<button type="button" class="btn-icon" onclick={() => onedit?.(row as unknown as UserItem)} title="Edit user" aria-label="Edit user">
							<Pencil size={12} />
						</button>
						<button type="button" class="btn-icon danger" onclick={() => ondelete?.(row as unknown as UserItem)} title="Delete user" aria-label="Delete user">
							<Trash2 size={12} />
						</button>
					</div>
				{:else}
