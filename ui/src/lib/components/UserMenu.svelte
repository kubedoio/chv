<script lang="ts">
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    switch (event.key) {
      ...
      case 'ArrowDown':
        event.preventDefault();
        focusIndex = (focusIndex + 1) % (menuItems.length + 1); // +1 for logout
        break;
      
      case 'ArrowUp':
        event.preventDefault();
        focusIndex = (focusIndex - 1 + menuItems.length + 1) % (menuItems.length + 1);
        break;
...
<div class="relative">
  ...
  {#if isOpen}
    ...
    >
      ...
      <button
        type="button"
        class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-300 hover:bg-red-500/15 hover:text-red-400 transition-colors duration-150 focus-visible:outline-none focus-visible:bg-red-500/15 focus-visible:text-red-400 {focusIndex === menuItems.length ? 'bg-red-500/15 text-red-400' : ''}"
        role="menuitem"
