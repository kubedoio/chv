...
pub async fn get_overview(
    ...
    let nodes_total = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM nodes")
        ...
        .await
        .unwrap_or_else(|e| {
            warn!(error = %e, "overview: failed to query nodes_total");
            0
        });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query maintenance_nodes");
        0
    });
    ...
    let vms_total = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM vms")
        ...
        .await
        .unwrap_or_else(|e| {
            warn!(error = %e, "overview: failed to query vms_total");
            0
        });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query vms_running");
        0
    });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query active_tasks");
        0
    });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query networks_healthy");
        0
    });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query alerts");
        Vec::new()
    });
    ...
    .await
    .unwrap_or_else(|e| {
        warn!(error = %e, "overview: failed to query recent_tasks");
        Vec::new()
    });
