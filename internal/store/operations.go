...
func updateOperation(ctx context.Context, q querier, op *models.Operation) error {
	sql := `
		UPDATE operations SET
			operation_type = $2, category = $3, status = $4, status_message = $5,
			resource_type = $6, resource_id = $7, actor_type = $8, actor_id = $9, node_id = $10,
			request_payload = $11, result_payload = $12, error_payload = $13,
	...
	_, err := q.Exec(ctx, sql,
		...
		op.ResourceType, op.ResourceID, op.ActorType, op.ActorID, op.NodeID,
		op.RequestPayload, op.ResultPayload, op.ErrorPayload,
		op.StartedAt, op.FinishedAt, op.UpdatedAt,
	)
	return err
}
...
func listOperations(ctx context.Context, q querier, filters map[string]interface{}) ([]*models.Operation, error) {
	...
	if filters != nil {
		...
		if opType, ok := filters["operation_type"].(string); ok && opType != "" {
			conditions = append(conditions, fmt.Sprintf("operation_type = $%d", argIdx))
